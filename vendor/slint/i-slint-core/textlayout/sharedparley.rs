...
fn clip_selection_to_viewport(
    selection_rect: PhysicalRect,
    visible_clip: PhysicalRect,
) -> Option<PhysicalRect> {
    selection_rect.intersection(&visible_clip)
}
...
pub fn draw_text_input(
    ...
    if render {
        // Large multi-line selections may contain tens of thousands of rectangles. Drawing
        // all off-screen rectangles can overflow renderer geometry and leak selection color
        // outside a scrolled TextInput. Submit only the part inside the active viewport clip.
        let visible_clip = item_renderer.get_current_clip() * scale_factor;
        layout.selection_geometry(selection_range, |selection_rect| {
            if let Some(visible_selection) =
                clip_selection_to_viewport(selection_rect, visible_clip)
            {
...
mod selection_clip_tests {
    ...
    #[test]
    fn text_input_selection_is_clipped_to_visible_viewport() {
        let viewport = PhysicalRect::new(
            PhysicalPoint::new(0.0, 0.0),
            PhysicalSize::new(100.0, 50.0),
        );
        let partial = PhysicalRect::new(
            PhysicalPoint::new(-10.0, 20.0),
            PhysicalSize::new(30.0, 40.0),
        );
        let outside = PhysicalRect::new(
            PhysicalPoint::new(0.0, 60.0),
            PhysicalSize::new(100.0, 20.0),
        );

        assert_eq!(
            clip_selection_to_viewport(partial, viewport),
            Some(PhysicalRect::new(
                PhysicalPoint::new(0.0, 20.0),
                PhysicalSize::new(20.0, 30.0),
            ))
        );
        assert_eq!(clip_selection_to_viewport(outside, viewport), None);
    }
}
